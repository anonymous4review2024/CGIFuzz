...
class SUTInstance:
    ...
    def init_gdb(self, config: ConfigParser) -> GDB:
        if 'GDB' in config and 'add_one' in config['GDB']:
            add_one = config['GDB']['add_one']
        else:
            add_one = False
        ...
        while retries > 0:
            try:
                ...
                gdb.connect()
                gdb.set_follow_fork_mode_parent()

                # stop_reason, stop_info = gdb.wait_for_stop(timeout=2)
                # assert stop_reason == 'stopped, no reason given', \
                #     f'{stop_reason=} {stop_info=}'

                # assert stop_reason == 'input request', \
                #     f'{stop_reason=} {stop_info=}'
    ...
    def __exit__(
        ...
        if hasattr(self, 'gdb'):
            # self.gdb.send('-interpreter-exec console "monitor exit"')
            # try:
            #     self.gdb.interrupt()
            # except Exception as e:
